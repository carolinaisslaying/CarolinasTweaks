# Carolina's Tweaks

[<img alt="Requires Create" height="56" src="https://cdn.jsdelivr.net/npm/@intergrav/devins-badges@3.2.0/assets/cozy/requires/create_vector.svg">](https://modrinth.com/project/create)
[<img alt="Available on Modrinth" height="56" src="https://cdn.jsdelivr.net/npm/@intergrav/devins-badges@3/assets/cozy/available/modrinth_vector.svg">](https://modrinth.com/project/carolinas-tweaks)
[<img alt="See me on GitHub" height="56" src="https://cdn.jsdelivr.net/npm/@intergrav/devins-badges@3/assets/cozy/social/github-singular_vector.svg">](https://github.com/carolinaisslaying)
[<img alt="Chat on Discord" height="56" src="https://cdn.jsdelivr.net/npm/@intergrav/devins-badges@3/assets/cozy/social/discord-singular_vector.svg">](https://discord.gg/VVTJhkKc4G)

A collection of minor adjustments and tweaks by Carolina to improve your game. This is the NeoForge version of the mod. The Fabric version is no longer the primary focus and I can make no guarantee that it will be maintained. This decision, despite my original intention to never provide a Forge version, is due to the state of Create Fabric which this mod was intended to work alongside.

**Requires the Create mod to work! See your specific version on the version page for the required version of Create.** For 1.20.1 versions of this mod, Create is optional.

## Features
This adds a collection of minor features, such as berry juice, bundle of sticks and compressed cocoa blocks. It also adds a gold painted door and trapdoor, similar to the [Supplementaries](https://github.com/MehVahdJukaar/Supplementaries) mod, but controllable by redstone in the same way as a typical wood door.

Furthermore, this mod adds several recipes for other mods, see the compatibility section below and the wiki for more information.

For more information, head over the [Carolina's Tweaks mod wiki](https://github.com/carolinaisslaying/CarolinasTweaks/wiki).

## Installation

Head over the Modrinth page or the releases page on this GitHub and download the latest version that corresponds with your Minecraft version. Then, drop the downloaded file into your mods folder, and if on a server, into your server's mods folder as well.

## Compatibility
This mod should be compatible with all mods. Additional recipes are added for the following mods:
* [Create](https://modrinth.com/project/create) (required)
* [Ad Astra](https://modrinth.com/mod/ad-astra) (optional)
* [Create: The Factory Must Grow](https://modrinth.com/mod/create-tfmg) (optional)

If you are using Linux and are having issues with your cursor on Ad Astra or other modded GUIs, I recommend looking at my other mod, [Wayland Cursor Fix](https://modrinth.com/mod/wayland-cursor-fix).

## Credits and Licence
This mod was created by Carolina Mitchell.

The gold painted door and trapdoor textures are works of [Plantkillable](https://github.com/Plantkillable) from the Supplementaries Team, and are the copyright of the Supplementaries Team. Supplementaries is a really cool mod, [check them out on Modrinth](https://modrinth.com/mod/supplementaries)!

Please _see_ the following for the licence:
* [ASSETS_LICENCE.md](https://github.com/carolinaisslaying/CarolinasTweaks/blob/1.21.1-neoforge/ASSETS_LICENCE.md) for non-programmatical components of the software.
* [LICENCE.md](https://github.com/carolinaisslaying/CarolinasTweaks/blob/1.21.1-neoforge/LICENCE.md) for all other parts of the software.
